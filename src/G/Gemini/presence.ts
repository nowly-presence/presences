...
const browsingDetails = (activity: string, strings: typeof enUS): string => {
  switch (activity) {
    case "gems":
      return strings.browsingGems
    case "library":
      return strings.browsingLibrary
    case "videos":
      return strings.browsingVideos
    case "images":
      return strings.browsingImages
    case "students":
      return strings.browsingStudents
    case "search":
      return strings.browsingSearch
    case "notebooks":
      return strings.browsingNotebooks
    default:
      return strings.browsingGemini
  }
}
...
presence.on("UpdateData", async (ctx) => {
  ...
  if (page.kind === "chat") {
    ...
    const data: PresenceData = {
      details: hideDetails ? strings.usingGemini : strings.viewingConversation,
      state: title,
  ...
  if (page.kind === "notebook") {
    ...
    await presence.setActivity({
      details: strings.viewingNotebook,
      state: title,
      largeImageKey: Assets.Logo,
      largeImageText: "Gemini",
      type: PresenceType.Playing,
