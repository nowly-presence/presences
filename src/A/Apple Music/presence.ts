...
const settings = Presence.Settings({
  ...
  showBrowsing: {
    ...
    description: {
      "en-US": "Show activity while you browse Apple Music with no track detected.",
      "fr-FR": "Affiche l'activité lorsque vous parcourez Apple Music sans titre détecté.",
      "es-ES": "Muestra actividad al explorar Apple Music sin una canción detectada.",
    },
  },
  showButtons: {
    type: "boolean",
...
const browsingDetails = (pathname: string, strings: typeof enUS): string => {
  ...
  if (path.startsWith("/library")) return strings.browsingLibrary
  return strings.browsingAppleMusic
}
...
presence.on("UpdateData", async (ctx) => {
  ...
  if (track) {
    ...
    const data: PresenceData = {
      ...
      largeImageText: privacy ? "Apple Music" : track.title,
