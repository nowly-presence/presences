...
import { getLeChatPage } from "./utils/page"
...
const settings = Presence.Settings({
  showPrivateChats: {
    ...
    description: {
      "en-US": "When enabled, private Le Chat conversations can show their title on Discord. Off by default: Discord only shows that you are using Le Chat.",
      "fr-FR": "Lorsque cette option est activée, les conversations privées Le Chat peuvent afficher leur titre sur Discord. Désactivé par défaut : Discord indique seulement que vous utilisez Le Chat.",
      "es-ES": "Si está activada, las conversaciones privadas de Le Chat pueden mostrar su título en Discord. Desactivada por defecto: Discord solo muestra que estás usando Le Chat.",
    },
  },
  ...
  showBrowsing: {
    ...
    description: {
      "en-US": "Show activity while you browse Le Chat outside a chat.",
      "fr-FR": "Affiche l'activité lorsque vous parcourez Le Chat hors d'un chat.",
      "es-ES": "Muestra actividad al explorar Le Chat fuera de un chat.",
    },
  },
  showButtons: {
    type: "boolean",
...
presence.on("UpdateData", async (ctx) => {
  ...
  const page = getLeChatPage()

  if (page.kind === "chat") {
    const hideDetails = page.private && !showPrivateChats
    const title = !hideDetails && showConversationTitle ? page.title : undefined
    const data: PresenceData = {
      details: title ? presence.formatString(strings.chatting, { title }) : strings.usingLeChat,
      largeImageKey: Assets.Logo,
      largeImageText: "Le Chat",
  ...
  await presence.setActivity({
    details: strings.browsingLeChat,
    largeImageKey: Assets.Logo,
    largeImageText: "Le Chat",
    type: PresenceType.Playing,
  })
